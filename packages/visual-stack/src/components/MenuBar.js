...
export const MenuBarDropdown = ({ children, title, open }) => (
  <li className={`dropdown ${open ? 'open' : ''}`}>
    ...
    <ul className="dropdown-menu user-menu">{children}</ul>
  </li>
);
...
export const MenuBarDropdownItem = ({ children }) => <li>{children}</li>;
    ...
    <div className="container-fluid">
      <div className="row">
        <div className="navbar-header navbar-header--logo">
          <span
            onClick={onTitleClick || always(undefined)}
            className={`navbar-brand ${onTitleClick && 'navbar-brand--button'}`}
          />
