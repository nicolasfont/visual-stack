...
export const DialogLayout = ({className, title, cancelButtonText, submitButtonText, disableSubmit, showSubmitButtonSpinner, logo, onCancel, onSubmit, children}) => (
  <div className={`vs-dialog-layout ${className ? className : ''}`}>
    ...
    <div className="vs-dialog-layout-footer">
      ...
      <Button id="vs-dialog-layout-submit" type="solid-primary" disabled={disableSubmit === true}
              onClick={onSubmit}>{getSubmitButtonText(submitButtonText, showSubmitButtonSpinner)}</Button>
    </div>
  </div>
);
